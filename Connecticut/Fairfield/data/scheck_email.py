...
# # merged_data['Email'] = merged_data['Email'].combine_first(merged_data['Email'])

# # # Memilih hanya kolom yang diperlukan untuk file akhir
# # final_data = merged_data[['County','Title','Name','Address','Contact','Email','Url']]
...
# # Jika nama kolom berbeda, ubah sesuai kebutuhan
# file_pertama.rename(columns={'email': 'Email'}, inplace=True)
# file_kedua.rename(columns={'email': 'Email'}, inplace=True)
...
# Membaca kedua file CSV
file_pertama = pd.read_csv('hasil-scrapingNew York -- email - Sheet1-1.csv')
file_kedua = pd.read_csv('hasil-sacrpingNew York -- email - Sheet1-2.csv')
