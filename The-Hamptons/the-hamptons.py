...
headers = {
    ...
    'dnt': '1',
    'priority': 'u=0, i',
    'referer': 'https://hamptonsmoms.com/resources',
...
# response = requests.get('https://hamptonsmoms.com/resources/apple-and-pumpkin-picking/',  cookies=cookies, headers=headers)
...
response = requests.get('https://hamptonsmoms.com/resources/kids-and-family-friendly-beaches/',  cookies=cookies, headers=headers)

soup = BeautifulSoup(response.content, 'html.parser')

results = []

title_name = 'Kid and Family Friendly Beaches'

page_section = soup.find_all('div', {'class': 'et_pb_toggle_content clearfix'})

for page_box_text in page_section:
    box_text = page_box_text.find_all('div', {'class':'box-text'})
    
    for box in box_text:
        # Ambil title dari <a> dalam <h4>
        h4_tag = box.find('h4')
        # print(h4_tag.text)
        ...
        #     # Menggunakan regex untuk mengambil alamat dan nomor telepon
        #     address_match = re.search(r"Address\s*:\s*(.+?)(?:Phone|$)", address_text)
        #     phone_match = re.search(r"Phone\s*:\s*(\S+)", address_text)
            
        #     # Mengambil alamat dan telepon dari hasil pencarian regex
        #     if address_match:
        ...
        results.append({
            'Title' : title_name,
            'Name': title,
            'Address': address.replace('Phone: ',' ').replace('Address: ',''),
            'Contact': '',
            'Email':'',
            'url': url
        })
    ...
    # Temukan semua elemen <div> dengan kelas "box-text"
    # boxes = soup.find_all('div', class_='box-text')
    # for box in page_box_text:
    #     # Ambil title dari <a> dalam <h4>
    #     h4_tag = box.find('h4')
    #     a_tag = h4_tag.find('a') if h4_tag else None
    #     title = a_tag.text.strip() if a_tag else None
    ...
    #     # Ambil address dan phone dari <p>
    #     p_tags = box.find_all('p')
    #     print(p_tags)
        # address, phone = None, None
        # for p in p_tags:
        #     # Cek apakah teks mengandung "Address" dan "Phone"
        #     text = p.get_text(separator=' ').strip()
        #     if "Address:" in text:
        #         # Ambil alamat setelah "Address:"
        #         address = text.split("Address:")[1].split("Phone:")[0].strip()
        #         phone = text.split("Phone:")[1].strip() if "Phone:" in text else None
